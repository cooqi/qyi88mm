<style scoped lang="scss">
  ...
  .classify dl dd.on{color: $MainColor;  font-weight: bold;}
  .classify dl dd.act{font-weight: bold;color: $MainColor; }
  ...
  <div class="easyLife">
    <div class="shop-head-box">
      <div class="classify">
        <dl class="mainClass">
          <dt>分类</dt>
          <dd class="main" :class="['nav'+index,mainId==item.mainId?'on':'']"  @click="link(index,item.mainId,item.servClass)" v-for="(item,index) in Nav"  v-text="item.mainName"></dd>
      ...
      <router-view :defId="defId"  :cl="'jjyl'"></router-view>
  ...
  export default {
    ...
    methods:{
      ...
      go(pamars){
           ...
           pamars.id=this.mainId!=''&&this.mainId?this.mainId:this.defId
            pamars.sid=this.sid
           this.mainId=pamars.id
