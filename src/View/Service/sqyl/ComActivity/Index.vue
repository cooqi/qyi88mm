        ...
        <div class="easyLife">
          <div class="shop-head-box">
            <div class="classify">
              <dl>
                <dt>类型</dt>
                <dd @click="OnSublist(0,1)" :class="sublistIndex1==0?'act':''"
                    style="margin-right: 15px">主题活动
                </dd>
                <dd @click="OnSublist(1,3)" :class="sublistIndex1==1?'act':''"
                    style="margin-right: 15px">节假日活动
                </dd>
              </dl>
              <div class="mainClass">
              ...
              <ul class="sqxm-row">
                <li class="sqxm-box" v-for="(item,index) in projects"
                  ...
                  <p><span class="sqxm-data">{{item.CreateTime}}</span><span class="sqxm-go"
                                                                             @click="$routeGo('sqhddetail',{id:item.id})">查看</span>
                  </p>
                </li>
              </ul>
  ...
  export default{
    ...
    methods: {
      ...
      loadData: function () {
        ...
        that.projects = [];
        if (arguments[0]) that.familyId = arguments[0];
        if (arguments[1]) that.type = arguments[1];
        api.get('ZhengfuFamilypension/Gov_ommunityApi', {Family_ID: that.familyId}).then(function (res) {
          var data = res.data;
          for (var i = 0; i < data.length; i++) {
            if (data[i].pubType == that.type) {
      ...
      link(i, id){
        this.navIndex = i
        console.log(i)
        $('.nav' + i).addClass('on').siblings().removeClass('on')
      ...
      OnSublist(i, sid){
        this.sublistIndex1 = i;
        this.type = sid;
        this.familyId='0000000000000000'
        this.loadData();
