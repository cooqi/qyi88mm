    ...
    methods: {
      loadDetail(){
        ...
        api.get('ZhengfuFamilypension/FamilypensionScreemOneApi', {}).then(function (res) {
          var familys = res.data;
          if (familys != null && familys.length > 0) {
            $.each(familys, function (index, family) {
              if (family.family_ID == that.$route.params.shopId) {
                that.info = family;
      ...
      renderMap(){
        ...
        var mp = new BMap.Map("mapDetail", {enableMapClick: false});
